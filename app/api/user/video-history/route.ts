...
export async function GET(request: NextRequest) {
  try {
    ...
    const user = await User.findById(session.user.id).select("videoHistory");

    if (!user) {
      return NextResponse.json({ error: "User not found" }, { status: 404 });
    }

    // Return video history sorted by timestamp (newest first)
    const videoHistory = (user.videoHistory || []).sort(
      (a, b) =>
...
// DELETE - Clear all video history
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Get all video URLs before clearing history
    const videosToDelete = user.videoHistory || [];
